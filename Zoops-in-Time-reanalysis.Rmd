...
ps.prune.LOCAL <- subset_taxa(ps.prune.LOCAL, (tax_table(ps.prune.LOCAL)[,"Order"]!="Chloroplast") | is.na(tax_table(ps.prune.LOCAL)[,"Order"]))
...
table(tax_table(PS.fin)[, "Kingdom"], exclude = NULL) # 305 Archaea, 9111 Bacteria
########### 
...
# rarify, and hellinger
PS.500.hell = transform_sample_counts(PS.500, function(x) sqrt(x / sum(x)))
...
richness.plot.spp <- plot_richness(PS.500, x="organism", color="organism", measures=c("Observed", "Shannon")) +
  labs(y= "Alpha Diversity Measure", x = "Zooplanktona and Water Samples") + 
...
```{r ASV Richness and Diversity stats and fig4}
# some stats
...
######## test whether there are effects of time and lake (lake is the big driver)
# Time and lake - shannon richness
rich.shan.time.lake<- lm(Shannon~ time_point + lake, data=Shan.rich.df)
Anova(rich.shan.time.lake, type=2) #lake significant

#  posthoc just looking at Lake (p=0.077)
...
#Eastern Brook   2.07 0.115 274     1.85     2.30  ab   
#Virginia        2.47 0.108 274     2.26     2.69   bc  
#Convict         2.57 0.121 274     2.33     2.81    c  
#Cooney          2.68 0.109 274     2.47     2.89    c  
#Blue            2.71 0.106 274     2.50     2.91    c  


#### by sample type
rich.shan.sample<- lm(Shannon~ sample_type, data=Shan.rich.df)
anova(rich.shan.sample) #sample signficant, always higher in water
...
Anova(rich.obs.all, type=2) #time*lake significant
...
#Holopedium   33.2 5.53 270     22.3     44.1  a    
#Daphnia      36.7 1.51 270     33.7     39.6  a    
#Calanoid     40.0 1.74 270     36.6     43.5  a    
#Cyclopoid    40.2 1.79 270     36.6     43.7  a    
#Water       112.7 2.74 270    107.3    118.1   b 
...
#  posthoc just looking at Lake
rich.test.lake-emmeans(rich.shan.all, pairwise~lake)
multcomp::cld(rich.test.lake, Letters=letters)
...
#Cyclopoid    2.19 0.0667 270     2.06     2.33  ab   
#Daphnia      2.41 0.0565 270     2.30     2.52   b   
#Water        3.99 0.1021 270     3.79     4.19    c  

# combine all richness from stats
Fig.4.rich.plot<-plot_grid(alpha.time.lake, richness.plot.spp,
                      ncol=2, rel_widths=c(9,11))
Fig.4.rich.plot
...
elevation.shan.plot<-ggplot(Shan.rich.df, aes(x=elevation_m, y=Shannon)) +
  geom_smooth(method = lm, color="dodgerblue3", alpha=0.2, fill="dodgerblue3")+
  geom_point(color="dodgerblue", size=1.5, alpha=0.6)+
...
pH.shan.plot<-ggplot(Shan.rich.df, aes(x=pH, y=Shannon)) +
  geom_smooth(method = lm, color="goldenrod", alpha=0.2, fill="goldenrod")+
  geom_point(color="goldenrod4", size=1.5, alpha=0.6)+
  ylab("Shannon diversity")+
  xlab("pH")+
  theme_classic()
...
DO.shan.plot<-ggplot(Shan.rich.df, aes(x=DO_perc, y=Shannon)) +
  geom_smooth(method = lm, color="seagreen", alpha=0.2, fill="seagreen")+
  geom_point(color="mediumseagreen", size=1.5, alpha=0.6)+
  ylab("Shannon diversity")+
...
cond.shan.plot<-ggplot(Shan.rich.df, aes(x=cond, y=Shannon)) +
  geom_smooth(method = lm, color="orchid", alpha=0.2, fill="orchid")+
  geom_point(color="purple", size=1.5, alpha=0.6)+
  ylab("Shannon diversity")+
  xlab("Conductivity (uS/cm)")+
  theme_classic()
 
multregres.plots<-plot_grid(elevation.shan.plot, pH.shan.plot, DO.shan.plot, cond.shan.plot,
                            ncol=4, nrow=1)
multregres.plots
dev.copy(pdf, "figures/Figx.Multregrss.pdf", height=3.5, width=11)
dev.off()
...
```{r Bray curtis distance matrix, eval=FALSE}
#################
...
NMDS.metad<-pssd2veg(PS.500.hell)
NMDS.OTU<-psotu2veg(PS.500.hell)
...
all.sam.df <- pssd2veg(PS.500.hell) # pull out the sample data for PERMANOVA
...
Run NMDS with envfit for daphnia, calanoids, cyclopoids to better understand how environment is impacting these host-microbiomes. We will run NMDS and fit env.fit, perform PERMANOVA, test betadispersion, and test env conditions on BC matrix. 
- First *Daphnia*
...
# run PERMANOVA
set.seed(781)
broad.model.daph <- adonis2(bc_daph.hell ~ lake, 
                            data = nmds.daph.rarhell.df, permutations = 999)
broad.model.daph # signif lake effect 
...
# betadispersion
anova(betadisper(bc_daph.hell, nmds.daph.rarhell.df$lake)) # not significant
```
...
# run PERMANOVA
set.seed(781)
broad.model.calan <- adonis2(bc_calan.hell ~ lake, 
                            data = nmds.calan.rarhell.df, permutations = 999)
broad.model.calan # signif lake effect 
...
# betadispersion
anova(betadisper(bc_calan.hell, nmds.calan.rarhell.df$lake)) # not significant
```
...
# run PERMANOVA
set.seed(781)
broad.model.cyclop <- adonis2(bc_cyclop.hell ~ lake, 
                            data = nmds.cyclop.rarhell.df, permutations = 999)
broad.model.cyclop # signif lake effect 
...
# betadispersion
anova(betadisper(bc_cyclop.hell, nmds.cyclop.rarhell.df$lake)) # not significant

```

- Now run NMDS-envfit for *Water*
```{r water env nmds}
# subset
...
NMDS.env.species<-plot_grid(NMDS.water + guides(color="none"),
                            NMDS.daph + guides(color="none"), 
                            NMDS.calan + guides(color="none"),
                            NMDS.cyclop + guides(color="none"),
                            extract.legend,
          ncol=5, rel_widths = c(10,9.2,10,10,1), labels = c("A","B", "C", "D", ""))
...
daph.phylo.rar<-subset_samples(PS.500, organism=="Daphnia")
...
prevalences <- seq(.05, 1, .05)
detections <- round(10^seq(log10(1e-3), log10(.4), length = 10), 3)
...
daph.core <- daph.core + theme_bw() + ylab("ASVs")
daph.core
...
```{r core calan}
#Zooplankton core taxa
calan.phylo.rar<-subset_samples(PS.500, organism=="Calanoid")
#keep only taxa with positive sums
...
```{r cyclo core}
#Zooplankton core taxa
cyclop.phylo.rar<-subset_samples(PS.500, organism=="Cyclopoid")
#keep only taxa with positive sums
...
pseq.core <- core(pseq.rel, detection = 0, prevalence = .5) # 17 taxa

# collapse rare taxa into other
pseq.core2 <- aggregate_rare(pseq.rel, "Genus", detection = 0, prevalence = .5) # 11 taxa

# core taxa and their names
core.taxa <- taxa(pseq.core)

#Total core abundance in each sample (sum of abundances of the core members
core.abundance <- sample_sums(core(pseq.rel, detection = 0.01, prevalence = .9))
...
detections <- seq(from = 0, to = round(max(abundances(pseq.rel))/10, -1), by = 1)
...
```
MAY NEED TO DELETE OR REPURPOSE THIS SECTION
```{r environmental correlations, eval=FALSE}
# USING RARIFIED-HELLINGER DATA
...
```
not necessary
```{r plotting lake elevations to visualize spatial differences, eval=FALSE}
...
dev.off()
```
leverage this in the other paper
```{r mantel test, geographic distance, eval=FALSE}
...
ORD.T1 <- ordinate(T1, method='NDMS', distance='bray')
...
nmds.time <- ggplot(nmds.rarhell.df, aes(x=NMDS1, y=NMDS2, color = time_point)) +
  geom_point(aes(color = time_point, shape=sample_type), size = 2) + 
...
nmds.lake <- ggplot(nmds.rarhell.df, aes(x=NMDS1, y=NMDS2)) +
  geom_point(aes(color = lake, shape=sample_type), size = 2) + 
...
nmds.basin <- ggplot(nmds.rarhell.df, aes(x=NMDS1, y=NMDS2))+
  geom_point(aes(color = basin, shape=sample_type), size = 2) + 
...
nmds.elevation <- ggplot(nmds.rarhell.df, aes(x=NMDS1, y=NMDS2, color = elev.cat)) +
  geom_point(aes(color = elev.cat, shape=sample_type), size = 2) + 
...
phy_group_plots_combined <- plot_grid(
  nmds.time, nmds.organism, nmds.phy_group,
  nmds.lake,  nmds.basin, nmds.elevation,  nrow=2, ncol=3, labels = c("A","B", "C", "D", "E", "F", ""))

phy_group_plots_combined
dev.print(pdf, "figures/Figx.NMDS.groupeffects.pdf", height=12, width=18)
dev.off()

```

code here for separating lakes all by time, may not be useful considering the limited time effect
...
combined.nmds.lakes
dev.print(pdf, "figures/combined.nmds.lakes.pdf", height=10, width=15)
dev.off()
...
capscale and redundancy analysis, may not be useful
...
# get NMDS 1-3
sp.scores.hell <- as.data.frame(scores(nmds.rarhell, display = "sites"))
...
# CAPSCALE
set.seed(10000)
# use 500-rarified, hellinger transformed bray-curits matrix 'bc_500.hell'
capscale_result <- capscale(bc_500.hell ~ cond + chla_ug.L + DO_perc + pH + temp_C + DOC + 
                              elevation_m, dfun = "distance", data = nmds.rarhell.df) 
# could also be ~ ., for all variables
capscale_model <- capscale(bc_500.hell ~ 1, data = nmds.rarhell.df) # intercept only

# fitting global model, then performing stepwise selection with ordiRstep, compare intercept only to all explanatory variable model
mod <- ordiR2step(capscale_model, scope = capscale_result)
mod$anova # best model is "all" variables based on R2 and AIC

Anova(mod) # no matter the order of the explanatory variables in the original formula, the output of this shows that cond, elevation, DO, pH, temp/DOC is most important (in order)
summary(mod)
plot(mod)

mod_scores <- as.data.frame(scores(mod, choices = c(1,2), display = "sites")) #the CAP axes
merge.cap<-merge(sample_data(PS.500.hell), mod_scores, by = 'row.names', all = TRUE) # the merged df
row.names(merge.cap)<-merge.cap$Row.names

#df
cap_scores <- as.data.frame(scores(capscale_model, display = "sites", scaling=2)) #the MDS axes
merge.cap<-merge(merge.cap, cap_scores, by = 'row.names', all = TRUE) # the merged df
merge.cap<-merge.cap[-1]

#biplot arrows
mod_bp <- as.data.frame(scores(mod, choices = c(1,2), display = "bp", scaling=2)) #the CAP axes
mod_bp$Env<-as.factor(rownames(mod_bp))

# Type 2 scaling
ordiplot(mod, display="lc", scaling = 2)


#plot it
CAP.plot <- ggplot(merge.cap, aes(x = CAP1, y = CAP2, color=organism:basin)) +
  geom_point(aes(shape=organism)) + 
  stat_ellipse(aes(linetype=basin)) +
  scale_color_manual(values=c("coral", "brown3", "goldenrod2", "darkolivegreen3", "mediumseagreen", "darkslategray3", "dodgerblue", "orchid", "darkorchid3")) +
  xlab("CAP1 (Environment)") + ylab("CAP2 (Environment)") +
  geom_segment(data = mod_bp,
               aes(x = 0, xend = CAP1, y = 0, yend = CAP2),
               arrow = arrow(length = unit(0.25, "cm")), colour = "gray20") +
  geom_text(data = mod_bp, 
            aes(x = CAP1, y = CAP2, label = Env), colour = "black",
            size = 3)+
  theme_bw()
CAP.plot

CAPxMDS.plot <- ggplot(merge.cap, aes(x = CAP1, y = MDS1, color = organism:basin)) +
  geom_point() + stat_ellipse(aes(linetype=basin)) +
  scale_color_manual(values=c("coral", "brown3", "goldenrod2", "darkolivegreen3", "mediumseagreen", "darkslategray3", "dodgerblue", "orchid", "darkorchid3")) +
...
testing dbRDA in phyloseq with env and ordination 'in house', not especially useful
```{r, eval=FALSE}
## tried this with dbRDA

#dbRDA
set.seed(10000) 
upr <- dbrda(bc_500.hell ~ temp_C + elevation_m + cond + chla_ug.L + DOC + DO_perc + pH, 
             data = nmds.rarhell.df)
lwr <- dbrda(bc_500.hell ~ 1, data = nmds.rarhell.df, na.action = 'na.omit')
# fitting global model, then performing stepwise selection with ordiRstep
mod_selec <- ordiR2step(lwr, scope = upr)
mod_selec$anova # best model is "all" variables based on R2 and AIC
...
dbRDA <- dbrda(formula = bc_500.hell ~ temp_C + elevation_m + cond + chla_ug.L + DOC + DO_perc + pH, 
               data = nmds.rarhell.df, na.action = "na.omit") %>% 
               scores(tidy = TRUE) %>% 
              split(.[["score"]])

plot(dbRDA, display=c('bp', 'wa'), scaling=1) #wa is another


#########

# Create ordination
dbrda_test = ordinate(PS.500.hell, method="RDA", distance="bray") #dbRDA

# Mimic biplot from example
p0 = plot_ordination(PS.500.hell, dbrda_test, color = "organism")
p0

# select envs. traits
env.df<-nmds.rarhell.df %>% 
  dplyr::select(temp_C, elevation_m, cond, chla_ug.L, DOC, DO_perc, pH)

plt1 <- gg_ordiplot(dbrda_test, groups = nmds.rarhell.df$organism, plot = FALSE, conf = 0.90)
plt2 <- gg_envfit(dbrda_test, env = env.df, groups = nmds.rarhell.df$organism, plot = FALSE)
plt2$plot + geom_path(data = plt1$df_ellipse, aes(x=x, y=y, color=Group)) +
  guides(color=guide_legend(title="phyloseq env dbRDA"))
```
...
pm <- psmelt(PS.500)
Abund <- aggregate(Abundance ~ sample_type + organism + phy_group + lake + time_point + sequencing_ID + Class, data=pm, FUN=sum)
...
Facet <- ggplot(Abund.final, aes(x = time_point, y = rel_abund, fill= Class, color = Class)) +
  ...
  scale_color_manual(values = c("red4", "orangered", "gold", "lightgreen", "turquoise3", "seagreen4", "lightskyblue", "tan", "navy", "royalblue3", "tan4", "lightsalmon", "gray54"))
Facet
dev.copy(pdf, "figures/Facet.pdf", height=10, width=12)
dev.off()
```

may not be necessry
```{r stacked bar jacrel}
