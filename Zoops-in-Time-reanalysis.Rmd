...
pacman::p_load('knitr', 'microbiome', 'phyloseq', 'tidyr', 'tidyverse', 'knitr', 'magrittr', 'effects', 'devtools',
               ...
               'ade4', 'fossil', 'picante', 'reshape', 'readr', 'corrr', 'Hmisc', "MASS", "FSA", "sciplot",
               "decontam","BiocManager", 'ggpubr', 'ggmap', "ggordiplots", "fossil", "SpiecEasi", "igraph", "huge", 
               "leaflet", "ggtree", "colorBlindness", "RColorBrewer","MicrobiotaProcess", "predict3d", "ggiraphExtra",
               "data.table", "betapart", "metagMisc", "naniar", "rstatix")
```

## Overview

### Envrironmental Data
- depth of the lakes for table
```{r depth data}
...
- dot-line plot of the common taxa
...
tot.dens.aov<-Anova(tot.dens.mod, type=2) #time point trend
...
stepAIC(lm(dens.row.sums ~ latitude + longitude + DO..perc + DOC + chla_ug.L + pH + cond + temp..C, data = counts)) 
tot.zoop.dens.mod<-lm(dens.row.sums ~ temp..C, data = counts)
tot.zoop.dens.aov<-Anova(tot.zoop.dens.mod, type=2)
plot(allEffects(shan.zoop.mod))
...
- area stacked bar plot
```{r stacked bar plot}
long_counts <- tidyr::pivot_longer(data = counts, cols = c(22:33), 
...
- zooplankton shannon stats
```{r stats on zoops}
# Shannon diversity
zoop.comm.div <- as.data.frame(vegan::diversity(counts[,c(22:33)], index="shannon"))
zoop.comm.div <- cbind(counts, zoop.comm.div)
names(zoop.comm.div)[46] <- "Shannon"

zoop.shan.mod<-lm(Shannon ~ time_point + lake, data = zoop.comm.div)
zoop.shan.aov<-Anova(zoop.shan.mod, type=2)
# no effect of time or lake on Shannon diversity of zoop community samples (p> 0.425)

stepAIC(lm(Shannon ~ latitude + longitude + DO..perc + DOC + chla_ug.L + pH + cond + temp..C, data = zoop.comm.div)) 
shan.zoop.mod<-lm(Shannon ~ latitude + cond + temp..C, data = zoop.comm.div)
shan.zoop.aov<-Anova(shan.zoop.mod, type=2)
plot(allEffects(shan.zoop.mod))

#ggscatter(zoop.comm.div, x = "temp..C", y = "Shannon",
#          add = "reg.line", conf.int = TRUE,  xlim =c(12,22), ylim=c(0.5,2.5),
#          cor.coef = TRUE, cor.method = "pearson", cor.coef.coord = c(12.5,2.4),
...
ps.prune.LOCAL<-readRDS("output/reanalysis/ps.prune_local.RDS") #8056 taxa of 328 samples
...
table(sample_data(PS.500)$sample_type) #281 samples: 28 water, 253 zooplankton, 1651 ASVs
...
########### ########### ########### ########### 
saveRDS(PS.500, "output/reanalysis/PS.500.RDS") # save phyloseq
########### ########### ########### ########### 
...
Here is the map of sampling locations. Don't run this, Maddy made a new map
```{r map, eval=FALSE}
gps <- as.data.frame(cbind(unique(sample_data(PS.500)$longitude), unique(sample_data(PS.500)$latitude)))
rownames(gps) <- unique(sample_data(PS.500)$lake)
...
Shan.rich.df.env<-Shan.rich.df %>% dplyr::select(latitude, temp_C, DOC, elevation_m, chla_ug.L, 
...
# model 
a.div.mod <- lm(Shannon ~ temp_C + DOC + elevation_m + chla_ug.L + pH + DO_perc + cond, data = Shan.rich.df.env)
a.div.AIC <- MASS::stepAIC(a.div.mod, direction = "both")

# best model with AIC -160.5 ====. Shannon ~ DOC + elevation_m + DO_perc + cond
shan.mod<-lm(Shannon ~ DOC + elevation_m + DO_perc + cond, data = Shan.rich.df.env)
summary(shan.mod)
shan.aov<-Anova(shan.mod, type=2)
plot(allEffects(shan.mod)) # shannon increases with elevation, cond, DO

# elevation model and plot
# not significant alone
shan.elev<-Anova(lm(Shannon ~ elevation_m, data = Shan.rich.df.env), type=2)

elev.shan.plot<-ggplot(Shan.rich.df.env, aes(x=elevation_m, y=Shannon)) +
...
# ggPredict(shan.mod, pred=DO_perc, modx=cond, show.point = FALSE, se=TRUE, xpos=0.5), wont work with elevation in model

#DO 
summary(lm(Shannon ~ DO_perc, data=Shan.rich.df.env)) # adjust-R2=0.103
shan.DO<-Anova(lm(Shannon ~ DO_perc, data = Shan.rich.df.env), type=2)
...
DO.shan.plot<-ggplot(Shan.rich.df, aes(x=DO_perc, y=Shannon)) +
  ...
  theme_classic() +
  annotate(geom="text", x=80, y=0.1, label="Adjusted-R2=0.103, p<0.001", size=3,
           color="black")

#cond
summary(lm(Shannon ~ cond, data=Shan.rich.df.env)) # adjust-R2=0.075
shan.cond<-Anova(lm(Shannon ~ cond, data = Shan.rich.df.env), type=2)
...
cond.shan.plot<-ggplot(Shan.rich.df, aes(x=cond, y=Shannon)) +
  ...
  theme_classic() +
  annotate(geom="text", x=125, y=0.1, label="Adjusted-R2=0.075, p<0.001", size=3,
           color="black")
 
multregres.plots.shannon.env<-plot_grid(DO.shan.plot, cond.shan.plot,
...
PCoA.and.Unifracs
dev.copy(pdf, "figures/FigS4.PCoA.Unifrac.pdf", height=4, width=12)
dev.off()
```
...
# Env model
env.model.all <- adonis2(bc_500 ~ latitude + elevation_m + temp_C + pH + cond + 
                            DOC + chla_ug.L + DO_perc, data = nmds.rar.df, permutations = 999)
env.model.all # latitude is best (0.05), elevation (0.03), DO (0.016)
...
broad.model.zoop <- adonis2(bc_500 ~ time_point*lake*organism, 
                            data = nmds.rar.df, method="bray", permutations = 999)
broad.model.zoop
...
broad.model.zoop.wunif <- adonis2(wunif_500 ~ time_point*lake*organism, 
                            data = nmds.rar.df, method="wunifrac", permutations = 999)
...
##### 
### Unifrac from phyloseq: unif_500
set.seed(71)
broad.model.zoop.unif <- adonis2(unif_500 ~ time_point*lake*organism, 
                            data = nmds.rar.df, method="unifrac", permutations = 999)
broad.model.zoop.unif
...
broad.model.daph <- adonis2(bc_daph ~ time_point * lake, 
                            data = nmds.daph.rar.df, permutations = 999)
...
env.model.daph <- adonis2(bc_daph ~ latitude + elevation_m + temp_C + pH + cond + 
                            DOC + chla_ug.L + DO_perc, data = nmds.daph.rar.df, permutations = 999)
...
broad.model.calan <- adonis2(bc_calan ~ time_point * lake, 
                            data = nmds.calan.rar.df, permutations = 999)
...
broad.model.cyclop <- adonis2(bc_cyclop ~ time_point * lake, 
                            data = nmds.cyclop.rar.df, permutations = 999)
...
env.model.water <- adonis2(bc_water ~ latitude + elevation_m + temp_C + pH + cond + 
                            DOC + chla_ug.L + DO_perc, data = nmds.water.rar.df, permutations = 999)
...
NMDS.env.species
dev.copy(pdf, "figures/Fig6.NMDS.env.specieswater.pdf", height=4, width=13)
dev.off()
```
...
dev.print(pdf, "figures/Fig5.NMDS.groupeffects.pdf", height=8, width=10)
dev.off()
```
...
Rel.abund.class
dev.copy(pdf, "figures/Fig7.Relabund.facet.pdf", height=10, width=12)
dev.off()
```
...
dev.copy(pdf, "figures/FigSx.Relabund.class.fam.pdf", height=8, width=10)
dev.off()
...
# best model
Best.gamma<-lm(Gamma.rel_abund.sum ~ cond, data = gammaproteobacteria)
summary(Best.gamma)
Anova(Best.gamma) # conductivity is most important, Gamma decline with conduct. overall

anova(lm(Gamma.rel_abund.sum ~ cond , data = gammaproteobacteria)) #p=0.040, R2=0.012
...
gamma_cond <- ggplot(gammaproteobacteria, aes(x=cond, y=Gamma.rel_abund.sum)) +
  ...
  annotate(geom="text", x=60, y=1.0, label="Adjusted-R2=0.012, p=0.040", size=2,
           color="black")
gamma_cond
...
step(FitAll.alpha, direction = "both") # best is temp_C + DO_perc + cond + chla_ug.L

# best model
Best.alpha<-lm(Alphapr.rel_abund.sum ~  DO_perc, data = alphaproteobacteria)
summary(Best.alpha)
Anova(Best.alpha) # DO is most important, cond less so. Alpha decline with DO and cond increasing, increase with temp
plot(allEffects(Best.alpha))

# plot it
alpha_DO <- ggplot(alphaproteobacteria, aes(x=DO_perc, y=Alphapr.rel_abund.sum)) +
  geom_smooth(method = lm, color="goldenrod2", alpha=0.2, fill="goldenrod2")+
  ...
  annotate(geom="text", x=75, y=1.0, label="Adjusted-R2=0.098, p<0.001", size=2,
           color="black")
alpha_DO
...
summary(Best.actin)
Anova(Best.actin) # chla is most important, nearly signif.  Actino increase with chla overall
...
actino_chla <- ggplot(actinobacteria, aes(x=chla_ug.L, y=Actinob.rel_abund.sum)) +
  ...
  ggtitle("Actinobacteria") + theme_classic() +
   annotate(geom="text", x=1, y=1.0, label="Adjusted-R2=0.112, p=0.032", size=2,
           color="black")
actino_chla 
...
step(FitAll.Bactero, direction = "both") # temp_C + DOC + elevation_m  best

# best model
Best.bactero<-lm(Bactero.rel_abund.sum ~ temp_C + DOC + elevation_m, data = bacteroidia)
summary(Best.bactero)
Anova(Best.bactero, type=2) # DOC signif, temp  signif, no elevation effect. Decline with DOC and temp
...
# temp
summary(lm(Bactero.rel_abund.sum ~ temp_C, data = bacteroidia))
anova((lm(Bactero.rel_abund.sum ~ temp_C, data = bacteroidia)))
...
bactero_temp <- ggplot(bacteroidia, aes(x=temp_C, y=Bactero.rel_abund.sum)) +
  ...
  theme(axis.text.y=element_text(size=14)) + theme(axis.title.y=element_text(size=16)) + 
  ggtitle("Bacteroidia") + theme_classic()+
  annotate(geom="text", x=16, y=1.0, label="Adjusted-R2=0.023, p=0.014", size=2,
           color="black")
bactero_temp 
...
#### pool the plots
Taxa.relabund.env.regr<-plot_grid(gamma_cond, alpha_DO, actino_chla, bactero_temp, ncol=4,
                                  labels=c("C", "D", "E", "F"))

######### combine shannon with the taxa-specific
plot_grid(multregres.plots.shannon.env, Taxa.relabund.env.regr, ncol=1, nrow=2, rel_heights = c(0.8,1))
dev.copy(pdf, "figures/Fig8.Shann.env.Taxa.relabund.regr.pdf", width=12, height=7)
dev.off()
```
...
hcsample <- get_clust(obj=PS.500, distmethod="bray",
                      method=NULL, hclustmethod="average")
# rectangular layout
cplot1 <- ggclust(obj=hcsample,
                  layout = "rectangular",
                  pointsize=1,
                  fontsize=0,
                  factorNames=c("organism")
              ) +
              scale_color_manual(values=c("coral", "gold", "dodgerblue")) +
...
```{r}
library("betapart")
mdf = psmelt(PS.500) # melt to a df


# Extract abundance matrix from the phyloseq object
OTU1 = as(otu_table(PS.500), "matrix")
# transpose if necessary
if(taxa_are_rows(PS.500)){OTU1 <- t(OTU1)}
# Coerce to data.frame
...
PS.pa<-phyloseq_standardize_otu_abundance(PS.500, method = "pa") # convert to presence absence

OTU.pa.500 = as(otu_table(PS.pa), "matrix")
if(taxa_are_rows(PS.pa)){OTU.pa.500 <- t(OTU.pa.500)}
OTUdf.pa = as.data.frame(OTU.pa.500)

beta.dis<-beta.pair(OTUdf.pa, index.family = "sorensen")

# Extract turnover,  nestedness components
turnover <- beta.dis$beta.sim # dissimilarity matrix accounting for spatial turnover (replacement), measured as Simpson pair-wise dissimilarity

nestedness <- beta.dis$beta.sne #dist object, dissimilarity matrix accounting for nestedness-resultant dissimilarity, measured as the nestedness-fraction of Sorensen pair-wise dissimilarity

sor<- beta.dis$beta.sor # dist object, dissimilarity matrix accounting for total dissimilarity, measured as Sorensen pair-wise dissimilarity (a monotonic transformation of beta diversity)

# seems to fit in NMDS

set.seed(520)
nmds.nest <- metaMDS(nestedness,
          distance = "sorensen",
          k = 4,
          maxit = 999, 
          wascores = TRUE)
plot(nmds.nest, type="t")

# get NMDS 1-3, we will use this later in NMDS plots
scores.nest <- as.data.frame(scores(nmds.nest, display = "sites"))
nmds.nest.df <- merge(sample_data(PS.pa), scores.nest, by = 'row.names', all = TRUE)

# make the plot
NMDS.nest <- ggplot(nmds.nest.df) +
  geom_point(mapping = aes(x = NMDS1, y = NMDS2, colour = organism), size=2) +
  #coord_fixed(ratio=0.8) +
  scale_color_manual(values=c("coral2", "gold3", "darkolivegreen3", "darkslategray4", "dodgerblue")) +
  stat_ellipse(level=0.95, linetype = 2, linewidth=0.6, aes(x = NMDS1, y = NMDS2, color=organism)) +
...
# get NMDS 1-3, we will use this later in NMDS plots
scores.turno <- as.data.frame(scores(nmds.turno, display = "sites"))
nmds.turno.df <- merge(sample_data(PS.pa), scores.turno, by = 'row.names', all = TRUE)

# make the plot
NMDS.truno <- ggplot(nmds.turno.df) +
  geom_point(mapping = aes(x = NMDS1, y = NMDS2, colour = organism), size=2) +
  #coord_fixed(ratio=0.8) +
  scale_color_manual(values=c("coral2", "gold3", "darkolivegreen3", "darkslategray4", "dodgerblue")) +
  stat_ellipse(level=0.95, linetype = 2, linewidth=0.6, aes(x = NMDS1, y = NMDS2, color=organism)) +
...
plot_grid(NMDS.nest +guides(color="none"), NMDS.truno, ncol=2, rel_widths=c(3.7,5))
dev.copy(pdf, "figures/nest.turnover.pdf", width=9, height=4)
dev.off()
```
