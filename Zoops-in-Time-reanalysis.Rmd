...
PCoA.hell.rar.fin<- plot_ordination(
  physeq=PS.500.hell, 
  ordination = ord.hell) +
  geom_point(aes(color = organism), size = 2) +    
  stat_ellipse(level=0.95, linetype = 2, aes(color=organism)) +
...
Run PERMANOVA and NMDS so we can fit envionmental vectors
...
set.seed(123)
vf <- envfit(nmds.rarhell, env.hell, perm = 999, na.rm = TRUE) 
# lots of significance, except elevation, pH, DOC, TDN,    highest significance lat/long, spc, cond

### grab vectors and put into df
# The segments are scaled to the r2 value. You can extract this information with scores. Then these lengths are further scaled to fit the plot. This is done with a multiplier that is analysis specific, and can be accessed using the command ordiArrowMul(en).
spp.scrs <- as.data.frame(scores(vf, display = "vectors"))*ordiArrowMul(vf) 
...
NMDS.env <- ggplot(nmds.rarhell.df) +
  geom_point(mapping = aes(x = NMDS1, y = NMDS2, colour = organism)) +
...
NMDS.env
dev.copy(pdf, "figures/Figx.NMDS.env.pdf", height=7, width=7)
dev.off()

###############
# run PERMANOVA with Time, Lake, Organisms
# use the bray-curtis dist from phyloswq: bc_500.hell
set.seed(781)
broad.model.full <- adonis2(bc_500.hell ~ time_point*lake*organism, 
                            data = nmds.rarhell.df, method="bray", permutations = 999)
broad.model.full
...
Run NMDS with envfit for daphnia, calanoids, cyclopoids to better understand how environment is impacting these host-microbiomes.
- First *Daphnia*
...
- Finally, run NMDS-envfit for cyclopoids
...
NMDS.env.species<-plot_grid(NMDS.daph + guides(color="none"), 
                            ...
                            extract.legend,
          ncol=4, rel_widths = c(10,9.2,10,1), labels = c("A","B", "C", ""))

NMDS.env.species
dev.copy(pdf, "figures/Figx.NMDS.env.species.pdf", height=5, width=15)
dev.off()
```
...
# run PERMANOVA
set.seed(781)
broad.model3 <- adonis2(bc_zoops ~ time_point*lake*organism, data = nmds.merge3, permutations = 999)
broad.model3

env.model3 <- adonis2(bc_zoops ~ latitude + elevation_m + temp_C + pH + cond + DOC + chla_ug.L + DO_perc, data = nmds.merge3, permutations = 999)
...
# run PERMANOVA
set.seed(781)
broad.model4 <- adonis2(bc_water ~ time_point * lake, data = nmds.merge4, permutations = 999)
broad.model4

env.model4 <- adonis2(bc_water ~ latitude + elevation_m + temp_C + pH + cond + DOC + chla_ug.L + DO_perc, data = nmds.merge4, permutations = 999)
...
dev.print(pdf, "figures/NMDS.groupeffects.pdf", height=12, width=18)
dev.off()
