...
Hell.tb.RDA<- plot_ordination(
  ...
  scale_color_manual(values=c("coral", "gold3", "mediumaquamarine", "olivedrab3", 
...
# we will use unweighted since we have low abundance taxa
...
ordination.tests<-plot_grid(Rare.culling + guides(color="none"), 
                      ...
                      Rar.PCoA,   
                      PCoA.rar.unw.unif +  guides(color="none"),
                      Hell.tb.RDA + guides(color="none"), ncol=3, rel_widths=c(4,4,5,4,4,4))
...
PCoA.rar.bray<- plot_ordination(
  ...
  stat_ellipse(level=0.95, linetype = 2, aes(color=organism)) +
  scale_color_manual(values=c("coral2", "gold3", "darkseagreen", "darkslategray4", "steelblue2")) +
  scale_shape_manual(values= c(1,16))+
  ggtitle("ALL taxa-500 Bray") +
  theme_classic()
PCoA.rar.bray$layers <- PCoA.rar.bray$layers[-1]

library("colorBlindness")
cvdPlot(PCoA.rar.W.unif) # see that the pallete is colorblind friendly
...
set.seed(341)
ord.unif <- ordinate(PS.500, method = 'PCoA', distance ="unifrac")
...
PCoA.and.Unifracs<-plot_grid(PCoA.rar.bray +  guides(color="none", shape="none"),
                            PCoA.rar.unw.unif +  guides(color="none",  shape="none"), 
                            PCoA.rar.W.unif + guides(color="none",  shape="none"), extract.legend.PCoA, ncol=4, rel_widths=c(5,5,5,2))

PCoA.and.Unifracs
dev.copy(pdf, "figures/PCoA.Unifrac.pdf", height=4, width=12)
dev.off()
```
...
richness.spp.shan <- plot_richness(PS.500, x="organism", color="organism", measures=c("Shannon")) +
  ...
  scale_fill_manual(values=c("coral2", "gold3", "darkseagreen", "darkslategray4", "steelblue2")) +
  scale_color_manual(values=c("coral2", "gold3", "darkseagreen", "darkslategray4", "steelblue2")) +
...
Fig.4.rich.plot
dev.copy(pdf, "figures/Fig.4.rich.plot.pdf", height=6, width=12)
dev.off()
...
#organism   emmean     SE  df lower.CL upper.CL .group
# Daphnia      32.8 1.40 267     30.0     35.5  a    
# Calanoid     33.1 1.59 267     30.0     36.2  a    
# Holopedium   33.2 5.06 267     23.3     43.2  ab   
# Cyclopoid    41.4 1.66 267     38.1     44.7   b   
# Water        99.8 2.51 267     94.8    104.7    c 
...
# best model with AIC -160.5 ====. Shannon ~ elevation_m + DO_perc + cond, elevation
shan.mod<-lm(Shannon ~ elevation_m + DO_perc + cond, data = Shan.rich.df.env)
summary(shan.mod)
...
shan.elev<-anova(lm(Shannon ~ elevation_m, data = Shan.rich.df.env))
...
#DO 
summary(lm(Shannon ~ DO_perc, data=Shan.rich.df.env)) # adjust-R2=0.105
DO.shan.plot<-ggplot(Shan.rich.df, aes(x=DO_perc, y=Shannon)) +
  geom_smooth(method = lm, color="gray30", alpha=0.2, fill="gray30")+
  geom_point(color="gray20", size=1.5, alpha=0.6)+
  ylab("Shannon diversity")+
  xlab("Dissolved Oxygen (%)")+
  theme_classic()DO.shan.plot<-ggplot(Shan.rich.df, aes(x=DO_perc, y=Shannon)) +
  geom_smooth(method = lm, color="gray30", alpha=0.2, fill="gray30")+
  geom_point(color="gray20", size=1.5, alpha=0.6)+
  ylab("Shannon diversity")+
  xlab("Dissolved Oxygen (%)")+
...
#cond
summary(lm(Shannon ~ cond, data=Shan.rich.df.env)) # adjust-R2=0.076
...
nmds.rar.df <- merge(sample_data(PS.500), scores.full, by = 'row.names', all = TRUE)
...
# use the bray-curtis dist from phyloseq: bc_500
set.seed(781)
...
anova(betadisper(bc_500, all.sam.df$lake)) #  not-signif (barely)
anova(betadisper(bc_500, all.sam.df$time_point)) #  significant *barely
anova(betadisper(bc_500, all.sam.df$organism)) # very significant
...
# rarefied
set.seed(1000)
bc_daph <- phyloseq::distance(daph.phylo, method = "bray") # rarefied and hellinger transformed
...
NMDS.daph <- ggplot(nmds.daph.rar.df) +
  ...
  geom_text(data = spp.daph.scrs.signif, aes(x = NMDS1, y = NMDS2, label = Envfact), color="lemonchiffon4",
            size = 3)+
  theme_classic()
...
# betadispersion
anova(betadisper(bc_daph, nmds.daph.rar.df$time)) # not signf
anova(betadisper(bc_daph, nmds.daph.rar.df$lake)) # not significant, (p=0.317)
```
...
NMDS.calan <- ggplot(nmds.calan.rar.df) +
  ...
  geom_segment(data = spp.calan.scrs.signif,
               aes(x = 0, xend = NMDS1, y = 0, yend = NMDS2),
               arrow = arrow(length = unit(0.25, "cm")), colour = "goldenrod4") +
  geom_text(data = spp.calan.scrs.signif, aes(x = NMDS1, y = NMDS2, label = Envfact), color="lemonchiffon4",
            size = 3)+
  theme_classic()
...
# betadispersion
anova(betadisper(bc_calan, nmds.calan.rarhell.df$time)) # NS
anova(betadisper(bc_calan, nmds.calan.rarhell.df$lake)) # significant (p=0.04))
```
...
NMDS.cyclop <- ggplot(nmds.cyclop.rar.df) +
  ...
  geom_segment(data = spp.cyclop.scrs.signif,
               aes(x = 0, xend = NMDS1, y = 0, yend = NMDS2),
               arrow = arrow(length = unit(0.25, "cm")), colour = "goldenrod4") +
  geom_text(data = spp.cyclop.scrs.signif, aes(x = NMDS1, y = NMDS2, label = Envfact), color="lemonchiffon4",
            size = 3)+
  theme_classic()
...
NMDS.water <- ggplot(nmds.water.rar.df) +
  ...
  geom_segment(data = spp.water.scrs.signif,
               aes(x = 0, xend = NMDS1, y = 0, yend = NMDS2),
               arrow = arrow(length = unit(0.25, "cm")), colour = "goldenrod4") +
  geom_text(data = spp.water.scrs.signif, aes(x = NMDS1, y = NMDS2, label = Envfact), color="lemonchiffon4",
            size = 3)+
  theme_classic()
...
NMDS.env.species
dev.copy(pdf, "figures/Figx.NMDS.env.specieswater.pdf", height=4, width=13)
dev.off()
```
...
nmds.phy_group <- ggplot(nmds.rar.df, aes(x=NMDS1, y=NMDS2, color = phy_group)) +
  geom_point(aes(color = phy_group, shape=sample_type), size = 1.5) + 
...
nmds.time <- ggplot(nmds.rar.df, aes(x=NMDS1, y=NMDS2)) +
  geom_point(aes(color = Date, shape=sample_type), size = 1.5) + 
...
nmds.lake.basin <- ggplot(nmds.rar.df, aes(x=NMDS1, y=NMDS2)) +
  geom_point(aes(color = lake, shape=int.basin.type), size = 1.5) + 
...
phy_group_plots_combined <- plot_grid(
  nmds.phy_group, nmds.time, nmds.lake.basin,  rel_widths = c(3,3,3,3),
  nrow=1, ncol=3, labels = c("A","B", "C"))

phy_group_plots_combined
dev.print(pdf, "figures/Figx.NMDS.groupeffects.pdf", height=4, width=13)
dev.off()

########## ########## ########## ########## ########## ########## 
########## combine the full NMDS here with the unifracs plot

Unifrac.NMDS <- plot_grid(
  PCoA.rar.W.unif + guides(color="none",  shape="none"), 
  nmds.organism + guides(color="none",  shape="none"), 
  extract.legend.PCoA, rel_widths = c(3,3,1),
  nrow=1, ncol=3, labels = c("A","B", ""))

Unifrac.NMDS
dev.print(pdf, "figures/Figx.Unifrac.NMDS.pdf", height=4, width=9)
dev.off()
```
...
overall.comp.class <- ggplot(Abund.final, aes(x=organism, y=rel_abund, fill=Class, color=Class)) +
  ...
  scale_fill_manual(values = c("red4", "orangered", "gold", "lightsalmon", "turquoise3", "seagreen4", "lightgreen", "lightskyblue", "royalblue3", "plum", "tan3")) +
  scale_color_manual(values = c("red4", "orangered", "gold", "lightsalmon", "turquoise3", "seagreen4", "lightgreen", "lightskyblue", "royalblue3", "plum", "tan3"))
overall.comp.class
...
# best model
Best.gamma<-lm(Gamma.rel_abund.sum ~ temp_C + pH + DOC + cond + chla_ug.L, data = gammaproteobacteria)
summary(Best.gamma)
Anova(Best.gamma) # conductivity is most important, DOC to lesser degree. Gamma decline with conduct. overall

anova(lm(Gamma.rel_abund.sum ~ cond , data = gammaproteobacteria))
...
Anova(Best.bactero) # DOC signif, temp  signif, no elevation effect. Decline with DOC and temp
...
# best model
Best.alpha<-lm(Alphapr.rel_abund.sum ~ temp_C + DO_perc + cond + chla_ug.L, data = alphaproteobacteria)
summary(Best.alpha)
...
anova(lm(Alphapr.rel_abund.sum ~ DO_perc , data = alphaproteobacteria))

# plot it
alpha_DO <- ggplot(alphaproteobacteria, aes(x=DO_perc, y=Alphapr.rel_abund.sum)) +
  geom_smooth(method = lm, color="gold2", alpha=0.2, fill="gold2")+
