...
author: "CB Wall, MG Perreault, et al., *Molecular Ecology* (2025)"
date: "6/15/2025"
output:
  html_document:
    code_folding: hide
...
![**Figure. 1**. Sierra Nevada Lakes sampled for zooplankton microbiomes and environmental conditons across a summer season.](output/maps/Fig1.map.3 panel2.jpg){width=67%}
 
</center>  

## Environmental Data
Load in the environmental field data and create plots for environmental conditions and water nutrients/chemistry across all lakes and 6 sampling points. 
- first remove outliers, check data structure, and add some carpentry to the metadata data frame.
- then load in nutrient and water chemistry data (collected *in situ*)
- then load in air temperatures and precipitations data from sampling stations in Mammoth Lakes. 

```{r depth data, results='hide', collapse=TRUE}
field.df<-read.csv("data/field_nutr_metadata.csv")
...
str(field.df)
field.df$TDP_ug.L<-as.numeric(field.df$TDP_ug.L)
field.df$TDN_ug.L<-as.numeric(field.df$TDN_ug.L)

## replace outliers in nutrient analysis
field.df<-field.df %>% 
  naniar::replace_with_na(replace = list(TDP_ug.L = 4102.48))
...
```{r nutrients, fig.show='hold', collapse=TRUE, fig.dim=c(10,6), fig.cap="**Figure 2.** Environmental and water chemistry metrics measured in each of the six lakes across five sampling time points."}
######## TDP

TP.plot<-ggplot(field.df, aes(x=date.simple, y=TDP_ug.L, group=lake, color=lake)) + 
  geom_point(size=2)+
  geom_line() + 
...
TN.plot<-ggplot(field.df, aes(x=date.simple, y=TDN_ug.L, group=lake, color=lake)) + 
  geom_point(size=2)+
  geom_line() + 
  scale_color_manual(values =c("darkslategray", "dodgerblue3", "#63A5CA", "lightsteelblue4","lightblue3", "#8BDDC4"))+
  xlab("Sampling Time Point") + 
...
DOC.plot<-ggplot(field.df, aes(x=date.simple, y=DOC_mg.L, group=lake, color=lake)) + 
  geom_line() +  
  geom_point(size = 2)+  
  scale_color_manual(values =c("darkslategray", "dodgerblue3", "#63A5CA", "lightsteelblue4","lightblue3", "#8BDDC4"))+
  xlab("Sampling Time Point") + 
...
chla.plot<- ggplot(field.df, aes(x= date.simple, y=chla_ug.L, group=lake, color=lake)) + 
  geom_line() +  
  geom_point(size = 2)+  
  scale_color_manual(values =c("darkslategray", "dodgerblue3", "#63A5CA", "lightsteelblue4","lightblue3", "#8BDDC4"))+
  xlab("Sampling Time Point") + 
...
DO.percent.plot<-ggplot(field.df, aes(x= date.simple, y=DO_perc, group=lake, color=lake)) + 
  geom_line() +  
  geom_point(size = 2)+ 
...
laketemp.plot<-ggplot(field.df, aes(x= date.simple,  y=temp_C, group=lake, color=lake)) + 
  geom_line() +  
  geom_point(size = 2)+ 
...
```{r weather air temp and precipitation, collapse=TRUE, fig.dim=c(5,5), fig.show='hold', results='hide', fig.cap="**Figure S1.**  Average air temperature and total daily precipitation across the summer season in which lake sampling occurred." }
...
```{r plot of zooplankton community dot-line, collapse=TRUE}
# load in the density data
# density is the number of zoops per liter
counts <- read.csv('data/zoop_community.csv')
...
### Env. effects
stepAIC(lm(dens.row.sums ~ latitude + elevation_m + DO_perc + log(DOC_mg.L) + chla_ug.L + pH + cond + temp_C, data = counts))
tot.zoop.dens.env.mod<-lm(dens.row.sums ~ latitude + elevation_m + DO_perc + cond + temp_C, data = counts)
tot.zoop.dens.env.aov<-Anova(tot.zoop.dens.env.mod, type=2)
#plot(allEffects(lm(dens.row.sums ~ temp_C, data = counts)))
```
...
```{r testing effects for individual taxa, collapse=TRUE, fig.show="hide"}
##### Daphnia
...
```{r combine zooplankton plot, results='hide', fig.show="hold", fig.dim=c(8,5), fig.cap="**Figure S3.** Changes in zooplankton densities across six mountain lakes across five sampling time points"}
...
```{r stacked density plot, results="hide", fig.dim=c(8,4), fig.cap="**Figure 3.** Relative abundances of zooplankton taxa (as proportion of total density per liter) in communities of six mountain lakes in California’s Eastern Sierra Nevada across five sampling time points."}
...
```{r stats on zoops, results='hide', fig.show='hide', resutls='hide'}
# Shannon diversity
...
stepAIC(lm(Shannon ~ latitude + elevation_m + DO_perc + log(DOC_mg.L) + chla_ug.L + pH + cond + temp_C, data = zoop.comm.div))

shan.zoop.env.mod<-lm(Shannon ~ latitude + cond + temp_C, data = zoop.comm.div)
shan.zoop.env.aov<-Anova(shan.zoop.env.mod, type=2)
plot(allEffects(lm(Shannon ~ temp_C, data = zoop.comm.div)))
```
...
## Microbiome Data 
Import Phyloseq data from DADA pipeline. The data will need to be cleaned and organized so that it can be processed for downstream analysis.
...
- We wil inspect read depth and rarefaction curves to determine how to standardize sampling depth. 
```{r loading in data and modifying metadata, results='hide'}
##########################################################
...
```{r total reads by species boxplots raw data, resuls='hide', fig.show='hide'}
#### Inspect the run: 
...
```{r Read depth plots and rarefaction curves, fig.dim=c(6,4), results='hide', fig.show='hold', fig.cap="**Figure S2**. Rarefaction curve of number of bacterial ASVs as a function of sequencing depth (the number of reads per sample)."}
...
```{r ordination tests compiled, results='hide', fig.show='hold', fig.dim=c(16,10), fig.cap="**Exploratory Figure**. Testing different approaches and identifying samples with low reads to be dropped."}
####
...
- plots here will be in **Figure 4 A-D**, printed below with beta dispersion. 
```{r alpha diversity plot_richness, fig.show='hide'}
...
- Use Shannon and Observed and test for time, lake, and organisms (species and or water) effects on alpha diversity
```{r ASV Richness and Diversity stats, fig.show='hide', results='hide'}
# some stats
...
```{r ASV Richness and Diversity compiled, fig.show='hide'}
...
- first run a correlation matrix to see what variables show high correlation and should not be included in muliple analyses or variance or StepAIC model selection.
```{r correlation matrix, fig.show='hold', fig.dim=c(6,5), fig.cap="**Exploratory Figure**. Correlation matrix for environmental conditions and geography."}
...
```{r Shannon and environ, fig.show='hide', results='hide'}
...
```{r microbial shannon univariate models, collapse=TRUE}
...
```{r Shannon and environ plots, fig.show='hide', collapse=TRUE}
### plots
...
```{r PCoA and W-Unifrac final, collapse=TRUE, fig.show='hold', fig.dim=c(12,5), fig.cap="**Figure S5**. Analysis of zooplankton-associated and free-living (water) bacteria community beta diversity using (A) principal coordinate analysis (PCoA) of Bray-Curtis dissimilarity and (B) weighted and (C) unweighted unifrac distance."}
...
- output from models here will be in *Table S 
```{r metaMDS, scores, and PERMANOVAS from BC distance matrices, results='hide'}
####################################
...
```{r beta distance to centroids, fig.show='hide', results='hide'}
######
...
```{r daph env NMDS, fig.show='hide', results='hide', collapse=TRUE}
...
```{r calanoid env nmds, fig.show='hide', collapse=TRUE}
# subset
...
```{r cyclopoid env nmds, fig.show='hide', collapse=TRUE}
# subset
...
```{r water env nmds, fig.show='hide', collapse=TRUE}
# subset
...
```{r envfit NMDS fig, fig.dim=c(13,4), results='hide', fig.cap="**Figure 8**. Non-metric multidimensional scaling (NMDS) ordination on Bray-Curtis dissimilarity matrix of bacterial communities associated with (A) Daphnia, (B) calanoid and (C) cyclopoid copepod zooplankton, or in (D) water across six lakes and five sampling time points."}
...
```{r stacked bar plots, results='hide', message='hide', fig.show='hold', fig.dim=c(12,10), fig.cap="**Figure 6**. Relative abundance of bacteria classes among Cladocera (Daphnia, Holopedium) and Copepoda (calanoid and cyclopoid copepods) zooplankton and water in six lakes of the Eastern Sierra Nevada mountains at five time points across the summer season."}
...
```{r phyloseq class family RA, fig.show='hold', results='hide', fig.cap="**Figure S4**. Relative abundance of bacterial taxa Families among Daphnia, Holopedium, calanoid and cyclopoid copepods, and bacterioplankton in water from lakes of the Eastern Sierra Nevada mountains."}
...
```{r testing effects of env. variables on rel. abund. and richness of dominant bacteria, collapse=TRUE, fig.show='hide'}
