...
author: "CB Wall, MG Perreault, et al."
date: "6/15/2025"
output:
  html_document:
    code_folding: hide
...
```{r nutrients, fig.show='hold', fig.dim=c(12,8), fig.cap="**Figure 2.** Environmental and water chemistry metrics measured in each of the six lakes across five sampling time points."}
######## TDP
...
```{r weather air temp and precipitation, fig.dim=c(4,5),fig.cap="**Figure S1.**  Average air temperature and total daily precipitation across the summer season in which lake sampling occurred." }
...
Now, that we have examined the *total density of zooplankton*, we wil test for differences in density of individual taxa.
- We will look at Daphnia, calanoid copepods, cyclopoid copepods, nauplii, asplanchna, and other less common groups
- make dot-line plots and run linear modesl
```{r testing effects for individual taxa, results='hide', fig.show="hide"}
##### Daphnia
...
- Compile all the dot-line plots for all zooplantkon and the common zooplankton taxa
```{r combine zooplankton plot, results='hide', fig.show="hold", fig.dim=c(10,6), fig.cap="**Figure S3.** Changes in zooplankton densities across six mountain lakes across five sampling time points"}
...
```{r stacked density plot, results="hide", fig.dim=c(10,6), fig.cap="**Figure 3.** Relative abundances of zooplankton taxa (as proportion of total density per liter) in communities of six mountain lakes in California’s Eastern Sierra Nevada across five sampling time points."}
...
```{r stats on zoops, results='hide', fig.show='hide', collapse=TRUE}
# Shannon diversity
...
```{r Read depth plots and rarefaction curves, fig.dim=c(7,5), fig.show='hold', fig.cap="**Figure S2**. Rarefaction curve of number of bacterial ASVs as a function of sequencing depth (the number of reads per sample)."}
...
```{r exploring ordination and what to rarefy to, fig.show='hide'}
...
```{r ordination tests compiled, fig.show='hold', fig.dim=c(16,10), fig.cap="**Exploratory Figure**. Testing different approaches and identifying samples with low reads to be dropped."}
####
...
```{r ASV Richness and Diversity stats, fig.show='hide'}
# some stats
...
```{r correlation matrix, fig.show='hold', fig.dim=c(7,5), fig.cap="**Exploratory Figure**. Correlation matrix for environmental conditions and geography."}
...
```{r Shannon and environ, fig.show='hide'}
...
```{r PCoA and W-Unifrac final, fig.show='hold', fig.dim=c(12,5), fig.cap="**Figure S5**. Analysis of zooplankton-associated and free-living (water) bacteria community beta diversity using (A) principal coordinate analysis (PCoA) of Bray-Curtis dissimilarity and (B) weighted and (C) unweighted unifrac distance."}
...
- calcualte distance to centroids from NMDS 1 and 2
```{r beta distance to centroids, fig.show='hide'}
######
...
```{r calanoid env nmds, fig.show='hide'}
# subset
...
```{r cyclopoid env nmds, fig.show='hide'}
# subset
...
```{r water env nmds, fig.show='hide'}
# subset
...
```{r envfit NMDS fig, fig.dim=c(13,4), fig.cap="**Figure 8**. Non-metric multidimensional scaling (NMDS) ordination on Bray-Curtis dissimilarity matrix of bacterial communities associated with (A) Daphnia, (B) calanoid and (C) cyclopoid copepod zooplankton, or in (D) water across six lakes and five sampling time points."}
...
```{r phyloseq class family RA, fig.show='hold', fig.cap="**Figure S4**. Relative abundance of bacterial taxa Families among Daphnia, Holopedium, calanoid and cyclopoid copepods, and bacterioplankton in water from lakes of the Eastern Sierra Nevada mountains."}
...
```{r testing effects of env. variables on rel. abund. and richness of dominant bacteria, fig.show='hide'}
