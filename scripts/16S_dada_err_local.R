...
write.csv(filt_out, file="output/reanalysis/filt_out.csv")
filt_out<-read.csv("output/reanalysis/filt_out.csv")
row.names(filt_out)<-filt_out$X
filt_out<-filt_out[-1]

#################################
#inspect quality score plots for the filtered reads
pdf("output/reanalysis/qual_profiles_filtF.pdf")
plotQualityProfile(filtFs, aggregate = TRUE)
dev.off()

# create pdf of quality profiles for reverse samples
pdf("output/reanalysis/qual_profiles_filtR.pdf")
...
saveRDS(errF, "output/reanalysis/errF.rds")
errF<- readRDS("output/reanalysis/errF.rds")

# error rate for reverse
errR <- learnErrors(filtRs, multithread=TRUE)
saveRDS(errR, "output/reanalysis/errR.rds")
errR<- readRDS("output/reanalysis/errR.rds")

# plot error rates
pdf("output/reanalysis/F_err.pdf")
plotErrors(errF, nominalQ=TRUE)
dev.off()

pdf("output/reanalysis/R_err.pdf")
...
saveRDS(derepFs, "output/reanalysis/derepFs.rds")
#derepFs<- readRDS("output/TSCC-output/derepFs.rds")

derepRs <- derepFastq(filtRs, verbose=TRUE)
saveRDS(derepRs, "output/reanalysis/derepRs.rds")
...
saveRDS(seqtab.nochim, "output/reanalysis/seqtab.nochim.rds")
...
write.table(summary_tab, "output/reanalysis/read_count_tracking_local.tsv", quote = FALSE, sep = "\t", col.names = NA)
