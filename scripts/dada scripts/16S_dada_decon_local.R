...
counts <- read.table(file = 'output/dada proc/ASV_counts.tsv', sep = '\t', header = TRUE, row.names = 1)
tax_table <- read.table(file = 'output/dada proc/ASV_taxonomy.tsv', sep = '\t', header = TRUE, row.names = 1)
asv_fasta <- readRDS("output/dada proc/ASV_fasta.rds")
...
write.csv(prev.ASVs, "output/dada proc/prev.ASVs_local.csv")


rich<-estimate_richness(ps.prune, split = TRUE, measures = NULL)
rownames(rich) <- gsub(x = rownames(rich), pattern = "X", replacement = "")
write.csv(rich, "output/dada proc/richness.table_local.csv")
...
########### 
saveRDS(ps.prune.local, "output/dada proc/ps.prune_local.RDS") # save phyloseq
ps.prune.local<-readRDS("output/dada proc/ps.prune_local.RDS") # bring it back
